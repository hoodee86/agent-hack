...
- 声明核心依赖：
  - `langgraph>=0.2`
  - `langchain-core>=0.2`
  - `langchain-openai` 或 `langchain-anthropic`（根据使用的 LLM）
  - `pydantic>=2.0`
...
### `check_read_only_tool(tool_name: str) -> None`

第一阶段只允许读类工具。若工具名不在 `{"list_dir", "read_file", "search_text"}` 内，抛出 `PolicyViolation`。
...
Planner 是 Agent 的推理核心，每次迭代都会调用 LLM，输出下一步计划和工具调用。
...
- 明确要求 Agent 输出结构化 JSON，包含：
  - `thought`：当前推理过程。
  - `plan`：当前完整计划列表（可更新）。
  - `current_step`：本次要执行的步骤描述。
  - `tool_call`：如果需要调用工具，提供工具名和参数；如果任务完成，设为 `null`。
  - `final_answer`：任务完成时的最终回答，否则为 `null`。

**实现要求**：

- 从 `state["observations"]` 构建观察历史，拼入消息。
- 使用 `with_structured_output` 或手动解析 LLM JSON 输出。
- 若 LLM 返回 `final_answer`，直接设置状态中的 `final_answer` 并跳过工具调用。
- 若 LLM 返回 `tool_call`，设置 `proposed_tool_call`，`risk_level` 根据工具名初步判断（读类工具默认 `"low"`）。
- 将 LLM 的 `thought` 和当前消息追加到 `state["messages"]`。
- 审计日志记录 `plan_update` 和 `tool_proposed` 事件。

**验收标准**：

- 给定用户目标后，Planner 能输出合法的 `plan` 和 `proposed_tool_call`。
- 当所有信息已足够时，Planner 能设置 `final_answer`。
- LLM 输出格式错误时不崩溃，记录错误并让 Reflector 处理。
...
- 捕获 `PolicyViolation`，将其作为 `deny` 决策，记录 `reason`。
- 更新 `state["risk_decision"]`。
- 审计日志记录 `policy_decision` 事件（包含决策和原因）。
...
- 拒绝时，在 `messages` 中追加一条系统消息，告知 Reflector 被拒绝的原因。
...
- 将工具结果摘要追加到 `state["messages"]`（便于 Planner 后续读取）。
