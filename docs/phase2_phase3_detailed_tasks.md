...
### T33 — Planner / Reflector 写操作闭环 已完成
...
- Planner prompt 已新增强约束：写前先收集证据，写后必须先跑 `run_command` 验证，再允许给最终回答。
- 当模型在写后试图直接结束时，graph 会二次提示要求验证；若仍未给出验证命令，则以“未验证写入”失败关闭，而不是伪造成功。
- Reflector 已识别写后验证命令、验证成功、验证失败自动回滚，以及审批拒绝后的替代建议文案。

### T34 — 阶段 3 单元测试、集成测试、安全回归 已完成
...
- 已新增 graph 回归，覆盖“写后不验证被阻断”“审批后验证成功”“验证失败自动回滚”。
- 已新增 integration 场景，覆盖“暂停审批 -> 批准恢复 -> 跑验证命令 -> 总结完成”。
- 已补 direct skill 安全回归，覆盖 symlink 越界写入拒绝，并保留既有二进制文件/超大 patch 拒绝测试。
- 当前全量测试为 `151 passed`。

### T35 — 阶段 3 验收与文档收口 已完成
...
- README 已补充审批恢复、写后验证、自动回滚和 `--rollback-run` 的使用说明。
- 设计文档已同步说明阶段 3 已落地的验证闭环与失败回滚行为。
- verbose `run_end` 已显示验证状态、验证命令和回滚结果，便于 CLI 验收。
