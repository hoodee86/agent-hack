...
本设计面向一个基于 LangGraph 的 Agent 原型。该 Agent 能在受控 Linux 环境中自主完成基础运维与开发类任务，包括读取文件系统、浏览目录、检索文件、编辑受限路径下的文件、执行 Linux 命令、观察结果并迭代决策。
...
- 提供基础 Linux skill：目录浏览、文件读取、文件写入、文本搜索、命令执行。
