...
- 基于 Web 的运行控制台：查看 execution timeline、model/tool 明细、pending approval，并在页面中 approve / reject
...
# 启动 Web 控制台
uv run linux-agent-web --config config.yaml --host 127.0.0.1 --port 8000
...
- 启动命令：`uv run linux-agent-web --config config.yaml`
- 默认地址：`http://127.0.0.1:8000`
- 当前支持的核心视图：
	- 运行列表与状态总览
	- 单个 run 的 audit timeline、tool / policy / approval 事件明细
	- `model_input` 里的 system / human / assistant / tool message 检视
	- pending approval 的 approve / reject 交互，以及 human decision note
	- 从页面直接发起新的 goal，并轮询查看执行进度
